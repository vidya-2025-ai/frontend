...
import { Plus, Bell, Search, Star, Video, Calendar as CalendarIcon, Clock, MapPin } from 'lucide-react';
...
const Schedule = () => {
  ...
  const { data: interviews, isLoading: interviewsLoading, refetch: refetchInterviews } = useQuery({
    ...
    queryFn: () => interviewService.getRecruiterInterviews(),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });

  // Fetch events
  const { data: events, isLoading: eventsLoading, refetch: refetchEvents } = useQuery({
    queryKey: ['recruiter-events'],
    queryFn: () => eventService.getRecruiterEvents(),
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
                    ...
                    <TabsContent value="events">
                      <div className="space-y-4">
                        ...
                        {eventsLoading ? (
                          ...
                          <div className="text-center py-8">
                            <CalendarIcon className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                            <p className="text-gray-500">No events planned for this date.</p>
                          </div>
                        ) : (
